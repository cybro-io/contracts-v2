...
abstract contract BaseLPManagerV3Pancake is IPancakeV3SwapCallback {
    ...
    struct PoolInfo {
        /// @notice Address of the Pancake V3 pool contract
        address pool;
    ...
    struct RawPositionData {
        ...
        /// @notice Accrued but uncollected token0 (per Pancake V3 positions storage)
        uint128 claimedFee0;
        /// @notice Accrued but uncollected token1 (per Pancake V3 positions storage)
        uint128 claimedFee1;
    }
    ...
    struct Position {
        ...
        /// @notice Previously accounted claimed token0 (tokensOwed0 in Pancake V3 storage)
        uint128 claimedFee0;
        /// @notice Previously accounted claimed token1 (tokensOwed1 in Pancake V3 storage)
        uint128 claimedFee1;
    ...
    /// @notice Pancake V3 NonfungiblePositionManager used for position operations
    ...
    /// @notice Pancake V3 factory used to resolve pool addresses
    ...
    constructor(INonfungiblePositionManager _positionManager, IProtocolFeeCollector _protocolFeeCollector) {
        ...
        factory = IPancakeV3Factory(_positionManager.factory());
    }

    /* ============ MODIFIERS ============ */

    /// @notice Restricts a call to the owner of `positionId` in the Pancake V3 position manager
    ...
    /**
     * @notice Retrieves all position data from Pancake V3 position manager
     ...
     * @param positionId The Pancake V3 position token id
    ...
    /**
     * @notice Reads current liquidity of a Pancake V3 position
        ...
        returns (int256 amount0, int256 amount1)
    {
        ...
        (amount0, amount1) = IPancakeV3Pool(poolInfo.pool)
            .swap(
                address(this),
                zeroForOne,
                int256(amount),
    ...
    /**
     * @notice Calculates unclaimed fees inside a range using Pancake V3 fee growth accumulators
     * @param pool Pool address
    ...
    function _calculateUnclaimedFees(
        ...
        uint256 feeGrowthGlobal0X128 = IPancakeV3Pool(pool).feeGrowthGlobal0X128();
        uint256 feeGrowthGlobal1X128 = IPancakeV3Pool(pool).feeGrowthGlobal1X128();
