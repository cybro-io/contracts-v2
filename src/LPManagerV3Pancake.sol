 ...
 */
contract LPManagerV3Pancake is BaseLPManagerV3Pancake {
    ...
    constructor(INonfungiblePositionManager _positionManager, IProtocolFeeCollector _protocolFeeCollector)
        BaseLPManagerV3Pancake(_positionManager, _protocolFeeCollector)
    {}
     ...
     * @param poolAddress Address of the Pancake V3 pool
    ...
    function getPoolInfo(address poolAddress) public view returns (PoolInfo memory) {
        return PoolInfo({
            ...
            fee: IPancakeV3Pool(poolAddress).fee()
        });
    }
    ...
    /**
     * @notice Creates a new Pancake V3 position with best-effort input rebalancing to maximize liquidity
