 ...
 */
contract AutoManagerV3Pancake is BaseLPManagerV3Pancake, EIP712, AccessControl {
    ...
    struct AutoCloseRequest {
        /// @notice Pancake V3 position NFT id to operate on
        uint256 positionId;
    ...
    constructor(
        ...
        address admin,
        address autoManager
    ) EIP712("AutoManager", "1") BaseLPManagerV3Pancake(_positionManager, _protocolFeeCollector) {
        oracle = _oracle;
        ...
        onlyRole(AUTO_MANAGER_ROLE)
    {
        ...
        (, int24 currentTick,,,,,) = IPancakeV3Pool(ctx.poolInfo.pool).slot0();
        int24 newLower;
        int24 newUpper;
        {
            int24 tickSpacing = IPancakeV3Pool(ctx.poolInfo.pool).tickSpacing();
     ...
     *      adjusts it by token decimals so that the result matches Pancake V3 sqrtPriceX96 semantics.
